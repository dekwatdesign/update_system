<?php
header('Content-Type: application/json');

require_once 'config_functions.php';

$config = loadConfig();

$api_url = "https://api.github.com/repos/{$config['repo_owner']}/{$config['repo_name']}/commits";

$ch = curl_init($api_url);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, [
    'User-Agent: PHP Script',
    "Authorization: token {$config['access_token']}",
    'Accept: application/vnd.github.v3+json'
]);

$response = curl_exec($ch);
$http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
curl_close($ch);

if ($http_code === 200) {
    $commits = json_decode($response, true);
    if (!empty($commits)) {
        $latest_commit_sha = $commits[0]['sha'];
        $local_commit_file = 'last_commit.txt';
        
        if (file_exists($local_commit_file)) {
            $local_commit_sha = trim(file_get_contents($local_commit_file));
            
            if ($local_commit_sha !== $latest_commit_sha) {
                echo json_encode(['status' => 'update_available']);
            } else {
                echo json_encode(['status' => 'up_to_date']);
            }
        } else {
            echo json_encode(['status' => 'update_available']);
        }
    } else {
        echo json_encode(['status' => 'error', 'message' => 'No commits found.']);
    }
} else {
    echo json_encode(['status' => 'error', 'message' => 'Unable to fetch commit information from GitHub.']);
}
?>